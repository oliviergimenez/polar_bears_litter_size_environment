...
# The best model without the individual traits + the individual traits as they are
# Dorinda's best model

model_1.1.2_D_4_individual_binomial <- nimbleCode({
  for(i in 1:N) {
    y[i] ~ dbin(p[i], 1)
    logit(p[i]) <- b[1] + 
      b[2] * ice_free_days_previous_s[i] +
      ...
      b[6] * size_s[i] +
      b[7] * size_s2[i] +
      b[8] * age_young[i] * day_s[i] +
      b[9] * age_old[i] * day_s[i] +
      eps1[year[i]]
  }
  for (i in 1:nbyear) {
    eps1[i] ~ dnorm(0, sd = sigma1)
...
#  ~ 2. Ice-free days + day capture + individual (no interac) ==================

# The best model without the individual traits + the individual traits as they are
# Dorinda's best model, except I removed the interaction term between age and day of 
# capture

model_1.1.2_D_4_individual_binomial_2 <- nimbleCode({
  for(i in 1:N) {
    y[i] ~ dbin(p[i], 1)
    logit(p[i]) <- b[1] + 
      b[2] * ice_free_days_previous_s[i] +
...
# ~ 3. Ice-free days + day capture + individual (no quadratic) ======================

# The best model without the individual traits + the individual traits as they are
# Dorinda's best model

model_1.1.2_D_4_individual_binomial_3 <- nimbleCode({
  for(i in 1:N) {
    y[i] ~ dbin(p[i], 1)
    logit(p[i]) <- b[1] + 
      b[2] * ice_free_days_previous_s[i] +
      ...
      b[5] * age_old[i] +
      b[6] * size_s[i] +
      b[7] * age_young[i] * day_s[i] +
      b[8] * age_old[i] * day_s[i] +
      eps1[year[i]]
  }
  for (i in 1:nbyear) {
    eps1[i] ~ dnorm(0, sd = sigma1)
...
# ~ 3. Ice-free days + day capture + individual (no quadratic & no interac) ======================

# The best model without the individual traits + the individual traits as they are
# Dorinda's best model, except I removed the interaction between age and day of capture
# and the quadratic effect of size

model_1.1.2_D_4_individual_binomial_4 <- nimbleCode({
  for(i in 1:N) {
    y[i] ~ dbin(p[i], 1)
    logit(p[i]) <- b[1] + 
      b[2] * ice_free_days_previous_s[i] +
...
# ~ 5. day capture + individual ================================

# The best model without the individual traits AND without ice-free days + the 
# individual traits as they are in Dorinda's best model.

model_1.1.2_D_4_individual_binomial_5 <- nimbleCode({
  for(i in 1:N) {
    y[i] ~ dbin(p[i], 1)
    logit(p[i]) <- b[1] + 
      b[2] * day_s[i] +
