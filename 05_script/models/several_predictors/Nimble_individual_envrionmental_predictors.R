...
model_1 <- nimbleCode({
  ...
  for(i in 1:10){
    b[i] ~ dnorm(0.00000E+00, sd = 10)
  } 
})


# . Individual traits + ice-free days * days (common effect) =========================
...
model_2 <- nimbleCode({
  ...
  for (i in 1:N) {
    ...
    log(q[i, 2]) <- b[1] + 
      ...
      b[7]*day_s[i] +                                          # Day of capture
      b[8]*age_young[i]*day_s[i] + b[9]*age_old[i]*day_s[i] +  # Interaction day of capture * age
      b[10]*ice_free_days_previous_s[i]*day_s[i] +             # Interaction day of capture * ice free days
      eps1[year[i]]                                            # Random effect of year
    ...
    log(q[i, 3]) <- b[11] + 
      ...
      b[7]*day_s[i] +
      b[8]*age_young[i]*day_s[i] + b[9]*age_old[i]*day_s[i] + 
      b[10]*ice_free_days_previous_s[i]*day_s[i] +
      eps1[year[i]]
...
model_1.1.2_D_common_5A <- function()  {
  ...
  for (i in 1:N) {
    ...
    log(q[i, 3]) <- b[8] +  b[9]*ice_free_days_previous_s[i] + b[3]*age_s[i] + b[4]*age2_s[i] + 
      b[5]*size_s[i] + b[6]*size2_s[i] + b[7]*day_s[i] + sigma1 * eta1[year[i]]
