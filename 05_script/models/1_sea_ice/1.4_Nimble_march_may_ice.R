#==============================================================================#
#                                                                              #
#                       Nimble Models for March-May Ice                        #
#                                                                              #
#==============================================================================#


# A. March-May ice t ==========================================================
# ~ 1. March-May ice t: effect 1c_VS_0c (1.4.1_D) ============================

model_1.4.1_D_effect_1c_VS_0c <- nimbleCode({
  
  for (i in 1:N) {
    y[i] ~ dcat(p[i, 1:J])
    log(q[i, 1]) <- 0
    log(q[i, 2]) <- a0 + a1*mar_may_ice_s[i] + eps1[year[i]]
...
# ~ 2. March-May ice t: effect 2-3c_VS_0c (1.4.1_D) =============================

model_1.4.1_D_effect_2_3c_VS_0c <- nimbleCode({
  
  for (i in 1:N) {
    y[i] ~ dcat(p[i, 1:J])
    log(q[i, 1]) <- 0
    log(q[i, 2]) <- a0 + eps1[year[i]]
    log(q[i, 3]) <- b0 + b1*mar_may_ice_s[i] + eps1[year[i]]
...
# ~ 3. March-May ice t: effect common (1.4.1_D) =============================

model_1.4.1_D_effect_common <- nimbleCode({
  
  for (i in 1:N) {
    y[i] ~ dcat(p[i, 1:J])
    log(q[i, 1]) <- 0
    log(q[i, 2]) <- a0 + a1*mar_may_ice_s[i] + eps1[year[i]]
    log(q[i, 3]) <- b0 + a1*mar_may_ice_s[i] + eps1[year[i]]
...
# ~ 4. March-May ice t: effect distinct (1.4.1_D) =============================

model_1.4.1_D_effect_distinct <- nimbleCode({
...
# ~ 5. March-May ice t: binomial =============================================

model_1.4.1_D_binomial <- nimbleCode({
  for(i in 1:N) {
    y[i] ~ dbin(p[i], n[i])
    logit(p[i]) <- b0 + b1 * mar_may_ice_s[i] + eps1[year[i]]
  }
  for (i in 1:nbyear) {
    eps1[i] ~ dnorm(0, sd = sigma1)
  }
